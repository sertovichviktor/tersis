...
// --- ОТДЕЛЬНЫЙ КОМПОНЕНТ ФОРМЫ (Чтобы не зависало) ---
...
function TersisApp() {
  ...
  return (
    <div className={`min-h-screen ${bg} relative overflow-hidden transition-colors duration-300`}>
          ...
          <div className="flex justify-between items-center h-18">
            <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="flex items-center focus:outline-none group">
              <div className="relative flex items-center justify-center h-10 w-10">
                <img src="https://tersis.lt/logo.png" alt="TERSIS" className="h-full w-full object-contain transition-transform duration-300 group-hover:scale-110" />
              </div>
        ...
        {isMenuOpen && (
          <div className={`md:hidden ${navBg} backdrop-blur-md border-t ${borderColor}`}>
            <div className="px-4 py-3 space-y-2">
              {(['services', 'fleet', 'about', 'coverage', 'contact'] as const).map((section) => (
                <button key={section} onClick={() => scrollToSection(section)} className={`block w-full text-left ${textSecondary} hover:text-[#0052ff] py-2 text-sm font-semibold`}>{t.nav[section]}</button>
              ))}
              <button onClick={() => scrollToSection('contact')} className="w-full bg-[#0052ff] text-white px-6 py-2.5 rounded-md font-semibold text-sm mt-2">{t.nav.getQuote}</button>
            </div>
          </div>
        )}
      </nav>
        ...
        <div className="max-w-7xl mx-auto w-full relative z-20">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="animate-fadeInUp text-left">
              ...
              <div className="flex flex-wrap gap-4"><button onClick={() => scrollToSection('contact')} className="bg-[#0052ff] text-white px-8 py-4 rounded-md text-base font-bold hover:bg-[#003dd6] transition flex items-center gap-2 uppercase tracking-wide shadow-lg">{t.hero.getQuote} <ArrowRight className="h-4 w-4" /></button><button onClick={() => scrollToSection('fleet')} className="border-2 border-white/20 text-white hover:bg-white/10 px-8 py-4 rounded-md text-base font-bold transition uppercase tracking-wide">{t.hero.fleetDetails}</button></div>
            </div>
      ...
      <section id="fleet" className={`py-24 px-4 sm:px-6 lg:px-8 border-t ${borderColor}`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            <div className={`border ${borderAccent} p-6 md:p-8 ${bgCard} rounded-xl flex flex-col`}>
              ...
              <div className="space-y-4 mb-8 flex-grow">
                {[ [t.fleet.length, '13.6 m'], [t.fleet.height, '2.7 m'], [t.fleet.capacity, `33 ${t.fleet.euroPallets}`], [t.fleet.idealFor, t.fleet.generalCargo] ].map(([label, value], i) => (
                  <div key={i} className={`flex justify-between items-start gap-4 ${i < 3 ? `border-b ${borderColor}` : ''} pb-3`}><span className={`${textMuted} text-[10px] md:text-sm uppercase tracking-widest font-bold`}>{label}</span><span className={`${textPrimary} font-black text-xs md:text-base`}>{value}</span></div>
                ))}
              </div>
              <div className={`pt-4 border-t ${borderColor}`}><p className={`${textMuted} text-xs italic`}>{t.fleet.standardFooter}</p></div>
            </div>
            <div className={`border ${borderAccent} p-6 md:p-8 ${bgCard} rounded-xl flex flex-col`}>
              <div className="flex items-center gap-4 mb-8">
              ...
              <div className="space-y-4 mb-8 flex-grow">
                {[ [t.fleet.internalHeight, '3.0 m'], [t.fleet.volume, '105 m³'], [t.fleet.specialization, t.fleet.lightweightCargo], [t.fleet.advantage, '+14% ' + t.fleet.capacity] ].map(([label, value], i) => (
                  <div key={i} className={`flex justify-between items-start gap-4 ${i < 3 ? `border-b ${borderColor}` : ''} pb-3`}><span className={`${textMuted} text-[10px] md:text-sm uppercase tracking-widest font-bold`}>{label}</span><span className={`${textPrimary} font-black text-xs md:text-base`}>{value}</span></div>
                ))}
              </div>
              <div className={`pt-4 border-t ${borderColor}`}><p className={`${textMuted} text-xs italic`}>{t.fleet.megaFooter}</p></div>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[ { icon: Shield, title: t.fleet.cmrInsured, sub: t.fleet.fullCoverage }, { icon: Clock, title: t.fleet.realTime, sub: t.fleet.tracking }, { icon: Truck, title: t.fleet.modernFleet, sub: '2018-2023' }, { icon: Check, title: t.fleet.euro6, sub: t.fleet.compliant } ].map((item, idx) => (
              <div key={idx} className={`border ${borderAccent} p-5 text-center ${bgCard} rounded-lg`}><item.icon className="h-7 w-7 text-[#0052ff] mx-auto mb-3" /><p className={`font-black ${textPrimary} text-xs mb-1 uppercase`}>{item.title}</p><p className={`text-xs ${textMuted}`}>{item.sub}</p></div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[ 
              ...
              { icon: Shield, title: lang === 'en' ? 'Cargo Safety' : 'Krovinių sauгumas', sub: lang === 'en' ? 'Guaranteed Protection' : 'Garantuota apsauga' },
      ...
      {/* ─── CONTACT / QUOTE (Isolating Form to Prevent Lag) ─── */}
      <section id="contact" className={`py-24 px-4 sm:px-6 lg:px-8 border-t ${borderColor}`}>
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12 animate-fadeInUp">
            <h2 className={`text-4xl md:text-5xl font-black ${textPrimary} mb-4 tracking-tight uppercase`}>{t.contact.title}</h2>
          ...
          <div className={`${bgCard} border ${borderAccent} rounded-2xl p-8 md:p-10 shadow-2xl animate-fadeInUp`}>
            {/* ВЫЗЫВАЕМ ОТДЕЛЬНЫЙ КОМПОНЕНТ ФОРМЫ */}
      ...
      <footer className={`${bg} border-t ${borderColor} py-16 px-4 sm:px-6 lg:px-8`}>
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-3 gap-12 mb-12">
            <div>
              <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="flex items-center mb-6 group text-left">
                <div className="relative h-10 w-10"><img src="https://tersis.lt/logo.png" alt="TERSIS" className="h-full w-full object-contain" /></div>
                <h4 className={`text-xl font-black ${textPrimary} ml-3`}>TERSIS</h4>
              </button>
              <div className={`space-y-4 ${textSecondary} text-sm`}>
                <div className="flex items-start gap-3"><MapPin className="h-5 w-5 text-[#0052ff] flex-shrink-0" /><div><p className={`font-bold ${textPrimary} uppercase text-xs mb-1`}>{t.footer.address}</p><p>Taikos pr. 141-305, Kaunas, Lithuania</p></div></div>
                <div className="flex items-start gap-3"><Phone className="h-5 w-5 text-[#0052ff] flex-shrink-0" /><div><p className={`font-bold ${textPrimary} uppercase text-xs mb-1`}>{t.footer.phone}</p><a href="tel:+37065955956">+370 65 955 956</a></div></div>
                <div className="flex items-start gap-3"><Mail className="h-5 w-5 text-[#0052ff] flex-shrink-0" /><div><p className={`font-bold ${textPrimary} uppercase text-xs mb-1`}>{t.footer.email}</p><a href="mailto:info@tersis.lt">info@tersis.lt</a></div></div>
              </div>
            </div>
            <div>
              <h5 className={`font-black ${textPrimary} mb-6 text-sm uppercase`}>{t.footer.servicesTitle}</h5>
              <ul className={`space-y-2 ${textSecondary} text-sm`}>{t.services.items.slice(0, 6).map((svc, i) => (<li key={i}><button onClick={() => scrollToSection('services')}>{svc.title}</button></li>))}</ul>
            </div>
            <div>
              <h5 className={`font-black ${textPrimary} mb-6 text-sm uppercase`}>{t.footer.legalTitle}</h5>
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-2 text-[#0052ff] font-black"><FileText className="h-4 w-4" /><span>LIC-009666-EBKR</span></div>
                <div className="flex items-center gap-2 text-green-400 font-bold"><Check className="h-4 w-4" /><span>{t.footer.cmrInsured}</span></div>
                <div className="flex items-center gap-2 text-[#0052ff] font-bold"><Check className="h-4 w-4" /><span>{t.footer.euroCompliant}</span></div>
              </div>
            </div>
          </div>
          <div className={`border-t ${borderColor} pt-8 text-center ${textMuted} text-xs uppercase`}><p>© 2026 TERSIS. All rights reserved.</p></div>
        </div>
      </footer>
    </div>
  )
