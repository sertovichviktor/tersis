...
function TersisApp() {
  ...
  return (
    <div className={`min-h-screen ${bg} relative overflow-hidden transition-colors duration-300`}>
      {/* ─── NAVIGATION (Layout 1:1 original) ─── */}
      ...
      {/* ─── HERO SECTION (With Video & Info Blocks) ─── */}
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center text-white">
            <div className="animate-fadeInUp">
              <div className="inline-block mb-6 px-4 py-2 bg-[#0052ff]/20 border border-[#0052ff]/40 rounded-md">
                <p className="text-[#0052ff] text-xs font-black tracking-widest uppercase">GLOBAL LOGISTICS NETWORK</p>
              </div>
              <h1 className="text-5xl sm:text-6xl md:text-[80px] font-black text-white mb-6 leading-[1.0] tracking-tight uppercase">
                OWN FLEET.<br /><span className="text-[#0052ff]">EUROPEAN</span><br />LOGISTICS.
              </h1>
              <p className="text-lg text-gray-200 mb-10 leading-relaxed max-w-lg font-medium">
                {lang === 'en' 
                  ? 'Operating 27+ modern Euro-6 vehicles. Specializing in high-capacity MEGA trailers (105 m³) and standard solutions worldwide.' 
                  : 'Valdome 27+ modernių Euro-6 transporto priemonių parką. Specializuojamės MEGA puspriekabėmis (105 m³) bei standartiniais sprendimais visame pasaulyje.'
                }
              </p>
            ...
            {/* RIGHT SIDE (Asset info instead of void) */}
            <div className="hidden md:flex flex-col gap-6 items-end">
               <div className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-xl w-full max-w-[320px]">
               ...
               <div className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-xl w-full max-w-[320px]">
                  ...
                  <p className="text-xl font-black text-white mb-1 uppercase tracking-tight">LIC-009666-EBKR</p>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[ { icon: Shield, title: t.fleet.cmrInsured, sub: t.fleet.fullCoverage }, { icon: Clock, title: t.fleet.realTime, sub: t.fleet.tracking }, { icon: Truck, title: t.fleet.modernFleet, sub: '2018-2023' }, { icon: Check, title: t.fleet.euro6, sub: t.fleet.compliant } ].map((item, idx) => (
              <div key={idx} className={`border ${borderAccent} p-5 text-center ${bgCard} rounded-lg hover:bg-[#0052ff]/5 transition`}><item.icon className="h-7 w-7 text-[#0052ff] mx-auto mb-3" /><p className={`font-black ${textPrimary} text-[10px] mb-1 uppercase`}>{item.title}</p><p className={`text-[10px] ${textMuted}`}>{item.sub}</p></div>
            ))}
          </div>
        </div>
      </section>

      {/* ─── SERVICES (Exactly as original) ─── */}
      ...
      {/* ─── ABOUT (Exactly as original) ─── */}
      <section id="about" className={`py-24 px-4 sm:px-6 lg:px-8 border-t ${borderColor}`}>
        <div className="max-w-7xl mx-auto text-center">
          <h2 className={`text-4xl md:text-5xl font-black ${textPrimary} mb-8 uppercase tracking-tight`}>{t.about.title}</h2>
          <div className="max-w-3xl mx-auto mb-16 space-y-4 text-gray-400 font-medium leading-relaxed">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 text-left">
            {t.about.features.map((f, idx) => {
              ...
              return ( <div key={idx} className={`border ${borderAccent} p-6 ${bgCard} rounded-xl hover:border-[#0052ff]/50 transition-all group`}><Icon className="h-10 w-10 text-[#0052ff] mb-4 group-hover:scale-110 transition" /><h3 className={`text-base font-black ${textPrimary} mb-1 tracking-tight`}>{f.title}</h3><p className={`text-sm ${textMuted}`}>{f.subtitle}</p></div> )
            })}
          </div>
        </div>
      </section>
      ...
      <section id="coverage" className={`py-24 px-4 sm:px-6 lg:px-8 border-t ${borderColor}`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className={`relative h-[450px] md:h-[650px] rounded-[40px] overflow-hidden border ${borderAccent} ${isDark ? 'bg-slate-900' : 'bg-slate-100'} flex items-center justify-center shadow-2xl`}>
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <span className="mt-4 bg-[#0052ff] text-white px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest italic shadow-xl">KAUNAS HUB</span>
            </div>
            <svg className="absolute inset-0 w-full h-full z-0 pointer-events-none" viewBox="0 0 1000 600" preserveAspectRatio="xMidYMid slice">
              <defs><linearGradient id="lineGrad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#0052ff" stopOpacity="0" /><stop offset="50%" stopColor="#0052ff" stopOpacity="1" /><stop offset="100%" stopColor="#0052ff" stopOpacity="0" /></linearGradient><marker id="arrow" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto"><path d="M0,0 L0,6 L6,3 z" fill="#0052ff" /></marker></defs>
      ...
      <footer className={`${bg} border-t ${borderColor} py-16 px-4`}>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 text-sm">
          ...
          <div className="flex gap-10"><Mail className="text-[#0052ff]" size={22} /><Phone className="text-[#0052ff]" size={22} /><MapPin className="text-[#0052ff]" size={22} /></div>
        </div>
      </footer>
    </div>
  )
