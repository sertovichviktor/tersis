...
// --- ИЗОЛЯЦИЯ: ГЕРОЙ БЕЗ ВИДЕО (Для теста фриза) ---
const HeroSection = memo(({ t, scrollTo }: any) => (
  <section id="top" className="relative h-screen flex items-center px-6 overflow-hidden bg-[#050a14]">
    {/* Заменили видео на градиент */}
    <div className="absolute inset-0 z-0 bg-gradient-to-br from-[#050a14] via-[#0a1628] to-[#050a14]" />
    <div className="relative z-10 max-w-7xl mx-auto w-full text-left">
      <div className="animate-fadeInUp">
        <h1 className="text-6xl md:text-8xl font-black uppercase leading-[0.85] text-white">
          {t.hero.title1}<br />
          <span className="text-[#0052ff]">{t.hero.title2}</span>
        </h1>
        <button onClick={() => scrollTo('contact-section-container')} className="mt-10 bg-[#0052ff] px-10 py-5 rounded-lg font-black text-sm uppercase tracking-widest text-white hover:bg-[#003dd6] transition-colors shadow-2xl">
...
const CoverageMap = memo(() => (
  <div className="relative h-[400px] md:h-[650px] rounded-[40px] overflow-hidden bg-black border border-white/5 shadow-2xl">
    <img src="/map-hub.jpg.png" className="absolute inset-0 w-full h-full object-cover opacity-60 pointer-events-none" />
  </div>
))
    ...
    <div className="bg-[#050a14] text-white min-h-screen">
      ...
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-[#050a14]/98 border-b border-white/5' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto flex justify-between items-center px-6 h-20 text-white">
          ...
          <div className="hidden md:flex gap-8 items-center">
            ...
            <button onClick={() => setLang(lang === 'en' ? 'lt' : 'en')} className="border border-white/20 px-3 py-1 rounded text-[10px] font-black uppercase">{lang}</button>
          </div>
      ...
      <section id="fleet" className="py-24 px-6 border-t border-white/5 bg-[#050a14]">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-center text-4xl md:text-5xl font-black mb-16 uppercase">Fleet Specifications</h2>
          <div className="grid md:grid-cols-2 gap-10">
            <div className="p-10 bg-[#0a1628] rounded-3xl border border-white/5 shadow-xl">
              <Truck className="text-blue-500 mb-6" size={40} />
              <h3 className="text-2xl font-black mb-4 uppercase">{t.fleet.standardClass}</h3>
              <p className="text-gray-400 text-sm mb-4">92 m³ • 33 Euro Pallets • 13.6 m</p>
              <p className="text-gray-500 italic text-xs pt-4 border-t border-white/5">{t.fleet.standardFooter}</p>
            </div>
            <div className="p-10 bg-[#0a1628] rounded-3xl border border-blue-500/20 shadow-xl">
              ...
              <p className="text-gray-400 text-sm mb-4">105 m³ • 3.0 m Height • MEGA Specialist</p>
      ...
      <section id="services" className="py-24 px-6 border-t border-white/5 text-center bg-[#050a14]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {t.services.items.slice(0, 8).map((s:any, i:number) => (
            <div key={i} className="p-8 bg-[#0a1628] rounded-2xl border border-white/5 hover:border-blue-500 transition-colors text-center">
      ...
      <section className="py-24 px-6 bg-[#050a14] border-t border-white/5 text-center text-white">
        <h2 className="text-4xl font-black mb-12 uppercase">International Hub</h2>
        <CoverageMap />
      </section>
