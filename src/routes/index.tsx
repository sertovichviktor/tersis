...
const MemoHero = memo(({ t, scrollTo }: any) => (
  <section className="relative h-screen flex items-center bg-[#050a14] overflow-hidden px-6">
    ...
    <div className="relative z-10 max-w-7xl mx-auto w-full">
      ...
      <button onClick={() => scrollTo('contact-form-isolated')} className="mt-10 bg-[#0052ff] text-white px-10 py-5 rounded-lg font-black text-sm uppercase tracking-widest hover:bg-[#003dd6] transition-colors shadow-2xl">
...
function TersisApp() {
  ...
  // ПРЕДОХРАНИТЕЛЬ: Запрещаем рендер от скролла, когда мы в форме
  const isFocusing = useRef(false)

  useEffect(() => {
    const handleScroll = () => {
      if (isFocusing.current) return; // Если мы в форме - игнорируем скролл-события
    ...
    <div className="min-h-screen bg-[#050a14] text-white font-sans">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 items-center text-xs font-bold uppercase tracking-widest">
            {['services','fleet','about','contact'].map(s => (
              <button key={s} onClick={() => scrollToSection(s === 'contact' ? 'contact-form-isolated' : s)} className="text-gray-400 hover:text-white transition-colors">{t.nav[s as keyof typeof t.nav]}</button>
            ))}
      ...
      <section id="fleet" className="py-24 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-4xl md:text-5xl font-black mb-16 uppercase">{t.fleet.title}</h2>
          <div className="grid md:grid-cols-2 gap-8 text-left">
            <div className="bg-[#0a1628] p-10 rounded-3xl border border-white/5">
              <Truck className="text-blue-500 mb-6" size={40} />
              <h3 className="text-2xl font-black mb-2 uppercase">{t.fleet.standardClass}</h3>
            ...
            <div className="bg-[#0a1628] p-10 rounded-3xl border border-blue-500/20">
              ...
              <h3 className="text-2xl font-black mb-2 uppercase text-blue-500">{t.fleet.megaAdvantage}</h3>
              <p className="text-gray-400 text-sm mb-4">105 m³ • 3.0m Height • MEGA</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {t.services.items.slice(0, 8).map((s:any, i:number) => (
            <div key={i} className="p-8 bg-[#0a1628] rounded-2xl border border-white/5 hover:border-blue-500 transition-all">
              {serviceIcons[i] && React.createElement(serviceIcons[i], { className: "text-blue-500 mb-4 mx-auto", size: 32 })}
      ...
      <section id="contact-form-isolated" className="py-32 px-6 bg-[#050a14]">
        <div className="max-w-4xl mx-auto">
