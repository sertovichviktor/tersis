...
// --- ИЗОЛЯЦИЯ: ВИДЕО (React его не тронет при клике) ---
  ...
  <section id="top" className="relative h-screen flex items-center px-6 overflow-hidden bg-[#050a14]">
    ...
    <div className="relative z-10 max-w-7xl mx-auto w-full">
      <div className="animate-fadeInUp text-left">
        ...
        <button onClick={() => scrollTo('contact-section-container')} className="mt-10 bg-[#0052ff] text-white px-10 py-5 rounded-lg font-black text-sm uppercase tracking-widest hover:bg-[#003dd6] transition-all shadow-2xl">
    ...
    <div className="bg-[#050a14] text-white min-h-screen overflow-x-hidden">
      ...
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-[#050a14]/98 border-b border-white/5 backdrop-blur-md' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto flex justify-between items-center px-6 h-20">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({top:0, behavior:'smooth'})}>
            ...
            <span className="font-black text-2xl tracking-tighter uppercase">TERSIS</span>
          </div>
      ...
      <section id="fleet" className="py-24 px-6 border-t border-white/5 bg-[#050a14]">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-4xl md:text-5xl font-black mb-20 uppercase tracking-tight">{t.fleet.title}</h2>
      ...
      <section className="py-24 px-6 bg-[#050a14] border-t border-white/5 text-center uppercase"><h2 className="mb-10 text-4xl font-black">Coverage</h2><MapBlock /></section>

      {/* ─── ФОРМА (ВОССТАНОВЛЕНА 1:1 СО СКРИНШОТА) ─── */}
