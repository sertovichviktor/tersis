...
function TersisApp() {
  ...
  return (
    <div className={`min-h-screen ${bg} relative overflow-hidden transition-colors duration-300`}>
          ...
          <div className="flex justify-between items-center h-18 py-4">
            ...
            <div className="flex items-center gap-4 md:gap-8">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setLang(lang === 'en' ? 'lt' : 'en')} className={`px-2 py-1.5 rounded border ${borderColor} ${textSecondary} text-[10px] font-bold uppercase`}>{lang === 'en' ? 'LT' : 'EN'}</button>
                ...
                <button onClick={() => scrollToSection('contact')} className="hidden sm:block bg-[#0052ff] text-white px-5 py-2 hover:bg-[#003dd6] transition font-bold text-sm uppercase tracking-wide rounded-md">{t.nav.getQuote}</button>
                <button className={`md:hidden ${textSecondary} ml-1`} onClick={() => setIsMenuOpen(!isMenuOpen)}>{isMenuOpen ? <X size={26} /> : <Menu size={26} />}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ─── HERO SECTION ─── */}
      <section className="relative h-screen min-h-[800px] flex items-center overflow-hidden bg-[#050a14]">
        ...
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 relative z-20">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="animate-fadeInUp text-white text-left">
              ...
              <h1 className="text-5xl sm:text-6xl md:text-[84px] font-black text-white mb-6 leading-[0.95] tracking-tight uppercase">
              ...
              <p className="text-lg text-gray-200 mb-10 leading-relaxed max-w-lg font-medium">
                {lang === 'en' 
                  ...
                  : 'Valdome 27+ modernių Euro-6 transporto priemonių parką. Specializuojamės MEGA puspriekabėmis (105 m³) bei standartiniais spредimais visame pasaulyje.'
                }
              </p>
            ...
            <div className="hidden md:flex flex-col gap-6 items-end">
               <div className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-2xl w-full max-w-[340px]">
                  <Truck className="text-[#0052ff] mb-4" size={42} />
                  <p className="text-5xl font-black text-white mb-1 leading-none uppercase text-left">27+</p>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest text-left">{lang === 'en' ? 'Modern Vehicles' : 'Modernūs vilkikai'}</p>
               </div>
               <div className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-2xl w-full max-w-[340px]">
                  <FileText className="text-[#0052ff] mb-4" size={42} />
                  <p className="text-xl font-black text-white mb-1 uppercase tracking-tighter leading-none text-left">LIC-009666-EBKR</p>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest text-left">{lang === 'en' ? 'EU Transport License' : 'ES transporto licencija'}</p>
               </div>
            </div>
          </div>
        </div>
      ...
      {/* ─── FLEET SPECIFICATIONS (Fully Restored) ─── */}
      <section id="fleet" className={`py-24 px-4 sm:px-6 lg:px-8 border-t ${borderColor}`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            {/* Standard Class */}
            <div className={`border ${borderAccent} p-8 ${bgCard} rounded-xl hover:border-[#0052ff]/50 transition-all duration-300 animate-slideIn`}>
              ...
              <div className="space-y-4 mb-8">
                {[
                  ...
                  <div key={i} className={`flex justify-between ${i < 3 ? `border-b ${borderColor}` : ''} pb-3`}>
                    <span className={`${textMuted} text-sm uppercase tracking-widest font-bold`}>{label}</span>
                    <span className={`${textPrimary} font-black`}>{value}</span>
                  </div>
                ))}
              </div>
            </div>
