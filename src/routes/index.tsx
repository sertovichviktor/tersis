...
// --- 1. ИЗОЛЯЦИЯ: ВИДЕО (Заглавная буква - чтобы не было белого экрана!) ---
  ...
  <section id="top" className="relative h-screen flex items-center px-6 overflow-hidden bg-[#050a14]">
    ...
    <div className="relative z-10 max-w-7xl mx-auto w-full">
      <div className="animate-fadeInUp">
        ...
        <button onClick={() => scrollTo('contact-section-container')} className="mt-10 bg-[#0052ff] px-10 py-5 rounded-lg font-black uppercase tracking-widest text-white hover:bg-[#003dd6] transition-colors">
    ...
    <div className="bg-[#050a14] text-white min-h-screen overflow-x-hidden">
      ...
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-[#050a14]/98 border-b border-white/5' : 'bg-transparent'}`}>
        ...
        {isMenuOpen && <div className="md:hidden bg-[#050a14] p-8 space-y-4 border-t border-white/5">
          {['services','fleet','about','contact'].map(s => <button key={s} onClick={() => scrollToSection(s === 'contact' ? 'contact-section-container' : s)} className="block w-full text-left text-gray-400 text-lg font-bold uppercase">{t.nav[s as keyof typeof t.nav]}</button>)}
        </div>}
      </nav>
      ...
      {/* ─── КОНТАКТНАЯ ФОРМА (ВОССТАНОВЛЕНА 1:1 ПО СКРИНШОТУ) ─── */}
