  ...
  <section id="top" className="relative h-screen flex items-center px-6 overflow-hidden bg-[#050a14]">
    ...
    <div className="relative z-10 max-w-7xl mx-auto w-full">
      <div className="animate-fadeInUp">
        <h1 className="text-5xl md:text-8xl font-black uppercase leading-[0.9] text-white">
          {t.hero.title1}<br />
          <span className="text-[#0052ff]">{t.hero.title2}</span>
        </h1>
        <button onClick={() => scrollTo('contact-section-container')} className="mt-10 bg-[#0052ff] px-10 py-5 rounded-lg font-black uppercase tracking-widest text-white hover:bg-[#003dd6]">
...
// --- ИЗОЛЯЦИЯ: КАРТА (УДАЛИЛИ АНИМАЦИИ SMIL - ГЛАВНЫЙ ВИНОВНИК ФРИЗА) ---
const CoverageMap = memo(() => (
  <div className="relative h-[400px] md:h-[650px] rounded-[40px] overflow-hidden bg-black border border-white/5">
    ...
    {/* Мы убрали теги <animate>, так как они вешают Chrome при фокусе на инпуты */}
    <svg className="absolute inset-0 w-full h-full z-10 pointer-events-none opacity-40" viewBox="0 0 1000 600">
      <path d="M 150,230 Q 300,100 485,205" stroke="#0052ff" strokeWidth="2" fill="none" />
      <path d="M 850,380 Q 650,250 485,205" stroke="#0052ff" strokeWidth="2" fill="none" />
    </svg>
  </div>
))
...
function TersisApp() {
  ...
  return (
    <div className="bg-[#050a14] text-white min-h-screen">
      {/* NAVIGATION */}
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-[#050a14] border-b border-white/5' : 'bg-transparent'}`}>
      ...
      <section id="fleet" className="py-24 px-6 border-t border-white/5 bg-[#050a14]">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-center text-4xl md:text-5xl font-black mb-16 uppercase">Fleet Specs</h2>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="p-10 bg-[#0a1628] rounded-3xl border border-white/5">
              <Truck className="text-blue-500 mb-4" size={40} />
              <h3 className="text-xl font-black mb-2 uppercase">{t.fleet.standardClass}</h3>
            ...
            <div className="p-10 bg-[#0a1628] rounded-3xl border border-blue-500/20">
              <Maximize2 className="text-blue-500 mb-4" size={40} />
              <h3 className="text-xl font-black mb-2 uppercase">{t.fleet.megaAdvantage}</h3>
      ...
      {/* --- CONTACT FORM (ИЗОЛИРОВАНА И БЕЗ AUTOFILL ГЛЮКОВ) --- */}
      <section id="contact-section-container" className="py-32 px-6 bg-[#050a14]">
          ...
          <div className="p-8 md:p-12 rounded-[32px] border border-[#1A2C45]" style={{ background: '#0F1A2B' }}>
            <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <label className="block text-[10px] font-black text-[#8899BB] uppercase tracking-widest mb-3">FROM (COUNTRY/CITY)</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-[#8899BB] uppercase tracking-widest mb-3">TO (COUNTRY/CITY)</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <label className="block text-[10px] font-black text-[#8899BB] uppercase tracking-widest mb-3">CARGO TYPE</label>
                  <input name="cargoType" placeholder="Electronics..." autoComplete="one-time-code" className="w-full bg-[#0a1628] border border-white/5 rounded-xl px-5 py-4 text-white text-sm outline-none focus:border-blue-500" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-[#8899BB] uppercase tracking-widest mb-3">WEIGHT (KG)</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <label className="block text-[10px] font-black text-[#8899BB] uppercase tracking-widest mb-3">VOLUME (M³)</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-[#8899BB] uppercase tracking-widest mb-3">NAME</label>
              ...
              <div>
                <label className="block text-[11px] font-black text-[#8899BB] uppercase tracking-widest mb-3">EMAIL</label>
              ...
              <div>
                <label className="block text-[11px] font-black text-[#8899BB] uppercase tracking-widest mb-3">PHONE</label>
              ...
              <div>
                <label className="block text-[11px] font-black text-[#8899BB] uppercase tracking-widest mb-3">MESSAGE (OPTIONAL)</label>
