...
function TersisApp() {
  ...
  return (
    <div className={`min-h-screen ${bg} relative overflow-hidden transition-colors duration-300`}>
        ...
        <div className="max-w-7xl mx-auto w-full relative z-20">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              {/* Заголовок всегда белый (text-white) */}
              <h1 className="text-5xl sm:text-6xl md:text-7xl font-black text-white mb-6 leading-[1.05] tracking-tight uppercase">
                OWN FLEET.<br />
                DIRECT IMPACT.<br />
                <span className="text-[#0052ff]">EUROPEAN</span><br />
                LOGISTICS.
              </h1>
              {/* Новый текст + светлый цвет + перевод */}
              <p className="text-lg text-gray-100 mb-10 leading-relaxed max-w-lg font-medium">
                {lang === 'en' 
              ...
              <div className="flex flex-wrap gap-4">
                <button onClick={() => scrollToSection('contact')} className="bg-[#0052ff] text-white px-8 py-4 rounded-md text-base font-bold hover:bg-[#003dd6] transition flex items-center gap-2 uppercase tracking-wide shadow-lg shadow-[#0052ff]/30">{t.hero.getQuote} <ArrowRight size={18} /></button>
            ...
            {/* Правая часть: инфо-блоки (вместо квадрата) */}
      ...
      {/* ─── FLEET SPECIFICATIONS (1:1 Original) ─── */}
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 uppercase font-black text-[10px] tracking-tight">
            {[ {icon:Shield,title:t.fleet.cmrInsured,sub:t.fleet.fullCoverage}, {icon:Clock,title:t.fleet.realTime,sub:t.fleet.tracking}, {icon:Truck,title:t.fleet.modernFleet,sub:'2018-2023'}, {icon:Check,title:t.fleet.euro6,sub:t.fleet.compliant} ].map((item, idx) => (
              <div key={idx} className={`border ${borderAccent} p-5 text-center ${bgCard} rounded-lg`}><item.icon className="h-7 w-7 text-[#0052ff] mx-auto mb-3" /><p className={textPrimary}>{item.title}</p><p className={textMuted}>{item.sub}</p></div>
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="about" className={`py-24 px-4 sm:px-6 lg:px-8 border-t ${borderColor}`}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12"><h2 className={`text-4xl md:text-5xl font-black ${textPrimary} mb-4 uppercase tracking-tight`}>{t.about.title}</h2></div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className={`relative h-[450px] md:h-[650px] rounded-[40px] overflow-hidden border ${borderAccent} ${isDark ? 'bg-[#050a14]' : 'bg-slate-100'} flex items-center justify-center shadow-2xl`}>
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <span className="mt-4 bg-[#0052ff] text-white px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest shadow-xl">KAUNAS HUB</span>
            </div>
      ...
      <footer className={`${bg} border-t ${borderColor} py-16 px-4`}>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex gap-8"><Mail className="text-[#0052ff]" size={22} /><Phone className="text-[#0052ff]" size={22} /><MapPin className="text-[#0052ff]" size={22} /></div>
        </div>
      </footer>
    </div>
  )
