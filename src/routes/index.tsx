...
const MemoHero = memo(({ t, scrollTo }: any) => (
  <section id="top" className="relative h-screen flex items-center px-6 overflow-hidden bg-[#050a14]">
    ...
    <div className="relative z-10 max-w-7xl mx-auto w-full">
      ...
      <button onClick={() => scrollTo('contact-section')} className="mt-10 bg-[#0052ff] px-10 py-5 rounded-lg font-black text-sm uppercase tracking-widest text-white hover:bg-[#003dd6] transition-colors">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center px-6 h-20 text-white">
          ...
          <div className="hidden md:flex gap-8 items-center">
            {['services','fleet','about','contact'].map(s => (
              <button key={s} onClick={() => scrollToSection(s === 'contact' ? 'contact-section' : s)} className="text-gray-400 hover:text-white text-xs font-bold uppercase tracking-widest">{t.nav[s as keyof typeof t.nav]}</button>
            ))}
            ...
            <form onSubmit={handleSubmit} className="space-y-6" autoComplete="off">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                <div>
                  ...
                  <input name="cargoType" placeholder="Electronics, Pallets, etc." className="w-full bg-[#0a1628] border border-white/5 rounded-xl px-5 py-4 outline-none focus:border-blue-500 transition-colors" />
                </div>
                <div>
              ...
              <div className="text-white">
                ...
                <textarea name="message" rows={4} placeholder="Additional details..." className="w-full bg-[#0a1628] border border-white/5 rounded-xl px-5 py-4 outline-none focus:border-blue-500 transition-colors resize-none"></textarea>
              </div>
