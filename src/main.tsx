      ...
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div className="animate-fadeInUp text-left">
          <div className="inline-block mb-6 px-4 py-2 bg-[#0052ff]/20 border border-[#0052ff]/40 rounded-md">
            <p className="text-[#0052ff] text-sm font-bold tracking-widest uppercase">{lang === 'en' ? 'EST. 2011 • Trusted Experience' : 'ĮKURTA 2011 • Patikima patirtis'}</p>
          </div>
...
const App = () => {
  ...
  return (
    <div className={`min-h-screen ${bg} ${textPrimary} transition-colors duration-300 antialiased overflow-x-hidden`}>
      ...
      {/* NAVIGATION */}
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-[#050a14]/95 backdrop-blur-md border-b border-white/5 h-16' : 'bg-transparent h-20'} flex items-center`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 items-center">
            {['services', 'fleet', 'about', 'contact'].map(s => <button key={s} onClick={() => scrollTo(s)} className="text-[11px] font-black uppercase tracking-widest text-gray-400 hover:text-white transition-colors">{(t.nav as any)[s]}</button>)}
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            <div className={`p-10 rounded-3xl border border-white/5 ${bgCard} text-left`}>
              <Truck className="text-[#0052ff] mb-6" size={40} /><h3 className="text-3xl font-black ${textPrimary} mb-4 uppercase">{t.fleet.standardClass}</h3>
              <div className="space-y-4 mb-8">
                <div className="flex justify-between border-b border-white/5 pb-2 text-xs font-bold uppercase text-gray-500"><span>{t.fleet.length}</span><span className="text-white">13.6 m</span></div>
                <div className="flex justify-between border-b border-white/5 pb-2 text-xs font-bold uppercase text-gray-500"><span>{t.fleet.height}</span><span className="text-white">2.7 m</span></div>
                <div className="flex justify-between border-b border-white/5 pb-2 text-xs font-bold uppercase text-gray-500"><span>{t.fleet.capacity}</span><span className="text-white">33 Euro Pallets</span></div>
              </div>
            ...
            <div className={`p-10 rounded-3xl border border-[#0052ff]/30 ${bgCard} text-left`}>
              <Maximize2 className="text-[#0052ff] mb-6" size={40} /><h3 className="text-3xl font-black ${textPrimary} mb-4 uppercase text-[#0052ff]">{t.fleet.megaAdvantage}</h3>
              <div className="space-y-4 mb-8">
                <div className="flex justify-between border-b border-white/5 pb-2 text-xs font-bold uppercase text-gray-500"><span>{t.fleet.internalHeight}</span><span className="text-white">3.0 m</span></div>
                <div className="flex justify-between border-b border-white/5 pb-2 text-xs font-bold uppercase text-gray-500"><span>{t.fleet.volume}</span><span className="text-white">105 m³</span></div>
                <div className="flex justify-between border-b border-white/5 pb-2 text-xs font-bold uppercase text-gray-500"><span>{t.fleet.advantage}</span><span className="text-white">+14% Capacity</span></div>
              </div>
      ...
      <section id="about" className={`py-24 px-4 border-t ${borderColor} text-center bg-[#050a14]`}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-gray-400 text-lg leading-relaxed mb-12 space-y-6">
            <p>{lang === 'en' ? 'TERSIS is a reliable logistics partner since 2011, operating a fleet of 27 Euro 6 vehicles.' : 'TERSIS yra patikimas logistikos partneris nuo 2011 m., valdantis 27 Euro 6 transporto priemonių parką.'}</p>
            <p className="font-bold text-white uppercase text-sm tracking-widest border border-white/10 inline-block px-4 py-2 rounded text-blue-500">License: LIC-009666-EBKR</p>
          </div>
      ...
      <footer className={`${bg} border-t ${borderColor} py-20 px-6`}>
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-16 text-left">
          <div className="space-y-6">
            ...
            <p className="text-gray-500 text-sm leading-relaxed">Asset-based carrier & international logistics provider since 2011. Your reliable hub for European transport solutions.</p>
          </div>
