...
import {
  ...
  Moon,
  Languages,
  Home, // <-- КРИТИЧНО: добавил импорт
} from 'lucide-react'
import { translations, type Lang } from './lib/i18n' // <-- Исправлено для Actions
        ...
        <div className="max-w-7xl mx-auto w-full relative z-20">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="animate-fadeInUp">
              ...
              <div className="flex wrap gap-4"><button onClick={() => scrollToSection('contact')} className="bg-[#0052ff] text-white px-8 py-4 rounded-md font-bold hover:bg-[#003dd6] transition flex items-center gap-2 uppercase tracking-wide"> {t.hero?.getQuote} <ArrowRight className="h-4 w-4" /></button></div>
            </div>
          </div>
        </div>
      </section>
