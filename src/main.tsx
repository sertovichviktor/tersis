...
const ContactInput = ({ label, id, name, type, placeholder, textSecondary, inputBg, borderAccent, textPrimary }: any) => {
  const [isFocused, setIsFocused] = useState(false);
  return (
    <div>
      <label htmlFor={id} className={`block text-xs font-bold ${textSecondary} mb-2 uppercase tracking-widest`}>
        {label}
      </label>
      <input
        id={id}
        ...
        autoComplete="off"
        className={`w-full px-4 py-3 ${inputBg} border ${isFocused ? 'border-[#0052ff] ring-1 ring-[#0052ff]/20' : borderAccent} ${textPrimary} outline-none transition-all duration-200 rounded-lg text-sm font-bold`}
  ...
  <div className="max-w-4xl mx-auto">
    ...
    <div className={`${bgCard} border ${borderAccent} rounded-2xl p-8 md:p-10 shadow-2xl animate-fadeInUp`}>
      <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-5">
        {/* ПЕРВЫЕ 6 ПОЛЕЙ (Используем быстрый ContactInput) */}
        {[
          { key: 'from', type: 'text' },
          { key: 'to', type: 'text' },
          ...
          <ContactInput
            ...
            name={key}
            type={type}
            label={(t.contact as any)[key] || key}
        ...
        {/* ПОЛЕ MESSAGE (Оставляем как было по дизайну, но добавляем ID) */}
        <div className="md:col-span-2">
