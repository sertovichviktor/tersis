    ...
    <div className="max-w-7xl mx-auto w-full relative z-20">
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div className="animate-fadeInUp text-left">
          <div className="inline-block mb-6 px-4 py-2 bg-[#0052ff]/20 border border-[#0052ff]/40 rounded-md text-[#0052ff] text-xs font-bold uppercase tracking-widest">
          ...
          <div className="flex gap-4">
            <button onClick={() => scrollToSection('contact')} className="bg-[#0052ff] text-white px-8 py-4 rounded-md font-bold hover:bg-[#003dd6] transition flex items-center gap-2 uppercase shadow-lg shadow-blue-500/20">
        ...
        <div className="hidden md:flex flex-col gap-6 items-start md:items-end">
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-xl w-full max-w-[340px]">
            <FileText className="h-8 w-8 text-[#0052ff] mb-4" /><p className="text-xl font-black text-white mb-1 uppercase tracking-tighter">LIC-009666-EBKR</p><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">EU License</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="relative h-[400px] md:h-[650px] rounded-[40px] overflow-hidden border border-[#0052ff]/30 shadow-2xl bg-black max-w-7xl mx-auto">
        ...
        <svg className="absolute inset-0 w-full h-full z-10 pointer-events-none" viewBox="0 0 1000 600" preserveAspectRatio="xMidYMid slice">
          <g fill="none" strokeWidth="2" strokeLinecap="round">
            ...
            <path d="M 850,380 Q 650,250 485,205" stroke="rgba(0,82,255,0.5)" strokeDasharray="1, 50"><animate attributeName="stroke-dashoffset" from="300" to="0" dur="3.5s" repeatCount="indefinite" /></path>
          </g>
        </svg>
      </div>
    </div>
...
function TersisApp() {
  ...
  return (
    <div className={`min-h-screen ${bg} ${textPrimary} transition-colors duration-300 font-sans overflow-x-hidden`}>
      ...
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-[#050a14]/95 backdrop-blur-md border-b border-white/5 h-16' : 'h-20'} flex items-center`}>
        <div className="max-w-7xl mx-auto px-6 w-full flex justify-between items-center text-white">
          ...
          <div className="hidden md:flex gap-8 items-center text-[11px] font-black uppercase tracking-widest text-gray-400">
            {['services', 'fleet', 'about', 'contact'].map(s => <button key={s} onClick={() => scrollToSection(s)} className="hover:text-[#0052ff] transition-colors">{t.nav[s as keyof typeof t.nav]}</button>)}
      ...
      <HeroSection t={t} lang={lang} scrollToSection={scrollToSection} />
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            <div className={`p-10 rounded-3xl border border-white/5 ${bgCard} text-left`}>
              <Truck className="text-[#0052ff] mb-6" size={40} /><h3 className="text-3xl font-black mb-4 uppercase">{t.fleet.standardClass}</h3>
              <div className="space-y-4 mb-8 text-[11px] font-black uppercase tracking-widest text-gray-500">
                <div className="flex justify-between border-b border-white/5 pb-2"><span>LENGTH</span><span className="text-white">13.6 M</span></div>
                <div className="flex justify-between border-b border-white/5 pb-2"><span>HEIGHT</span><span className="text-white">2.7 M</span></div>
                <div className="flex justify-between border-b border-white/5 pb-2"><span>CAPACITY</span><span className="text-white">33 PALLETS</span></div>
              </div>
            ...
            <div className={`p-10 rounded-3xl border border-[#0052ff]/30 ${bgCard} text-left`}>
              <Maximize2 className="text-[#0052ff] mb-6" size={40} /><h3 className="text-3xl font-black mb-4 uppercase text-[#0052ff]">{t.fleet.megaAdvantage}</h3>
              <div className="space-y-4 mb-8 text-[11px] font-black uppercase tracking-widest text-gray-500">
                <div className="flex justify-between border-b border-white/5 pb-2"><span>INTERNAL HEIGHT</span><span className="text-white">3.0 M</span></div>
          ...
          {t.services.items.map((s:any, i:number) => {
            ...
            return (
              <div key={i} className="p-8 rounded-3xl border border-white/5 bg-[#050a14] hover:border-[#0052ff] transition-all group">
                ...
                <h4 className="font-black text-[11px] uppercase tracking-widest leading-tight text-white">{s.title}</h4>
              </div>
            )
          })}
        </div>
      ...
      <section id="about" className={`py-24 px-4 border-t ${borderColor} text-center bg-[#050a14]`}>
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-400 text-lg leading-relaxed mb-12">TERSIS is a reliable logistics partner since 2011. License: <span className="text-white font-bold">LIC-009666-EBKR</span>. Modern fleet of 27 Euro 6 vehicles.</p>
           ...
           <div className="p-8 md:p-12 rounded-[40px] shadow-2xl border border-[#1A2C45]" style={{ background: '#0F1A2B' }}>
            <form onSubmit={handleSubmit} className="space-y-8 text-left" autoComplete="off">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                <div><label className="block text-[11px] font-black text-[#8899BB] uppercase tracking-widest mb-3">FROM (COUNTRY/CITY)</label><input name="from" required placeholder="Kaunas, Lithuania" className="w-full px-5 py-4 bg-[#0a1628] rounded-xl border border-white/5 outline-none focus:border-blue-500 transition-colors" /></div>
                <div><label className="block text-[11px] font-black text-[#8899BB] uppercase tracking-widest mb-3">TO (COUNTRY/CITY)</label><input name="to" required placeholder="Berlin, Germany" className="w-full px-5 py-4 bg-[#0a1628] rounded-xl border border-white/5 outline-none focus:border-blue-500 transition-colors" /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
      ...
      <footer className="py-20 bg-[#050a14] border-t border-white/5 text-center px-4 text-gray-600 text-[10px] font-bold uppercase tracking-widest leading-loose">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-16 text-left">
          <div className="space-y-6">
            <div className="flex items-center gap-2"><img src="https://tersis.lt/logo.png" className="h-8"/><span className="text-xl font-bold uppercase text-white tracking-tighter">TERSIS</span></div>
            <p>Asset-based carrier since 2011. Reliable European logistics specialist.</p>
          </div>
