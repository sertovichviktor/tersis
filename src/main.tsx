  ...
  <section className="relative h-screen flex items-center px-4 sm:px-6 lg:px-8 overflow-hidden bg-[#050a14]">
    ...
    <div className="max-w-7xl mx-auto w-full relative z-20 grid md:grid-cols-2 gap-12 items-center">
      <div className="animate-fadeInUp text-left">
        ...
        <p className="text-lg text-gray-200 mb-10 leading-relaxed max-w-lg font-medium">{t.hero.desc}</p>
        <div className="flex gap-4">
          <button onClick={() => scrollToSection('contact-section-container')} className="bg-[#0052ff] text-white px-8 py-4 rounded-md font-bold hover:bg-[#003dd6] transition flex items-center gap-2 uppercase shadow-lg shadow-blue-500/20">{t.hero.getQuote} <ArrowRight className="h-4 w-4" /></button>
...
const MemoMap = memo(({ t }: any) => (
  <section id="coverage" className="py-24 px-4 border-t border-white/5 bg-[#050a14]">
    <div className="max-w-7xl mx-auto text-center">
      ...
      <div className="relative h-[400px] md:h-[650px] rounded-[30px] overflow-hidden border border-[#0052ff]/30 shadow-2xl bg-black max-w-7xl mx-auto">
        ...
        <svg className="absolute inset-0 w-full h-full z-10 pointer-events-none" viewBox="0 0 1000 600" preserveAspectRatio="xMidYMid slice">
          <g fill="none" strokeWidth="2" strokeLinecap="round">
            ...
            <path d="M 220,480 Q 350,300 485,205" stroke="rgba(0,82,255,0.5)" strokeDasharray="1, 50"><animate attributeName="stroke-dashoffset" from="300" to="0" dur="3.5s" repeatCount="indefinite" /></path>
    ...
    <div className={`min-h-screen ${bg} ${textPrimary} transition-colors duration-300 antialiased overflow-x-hidden`}>
      ...
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-[#050a14]/95 backdrop-blur-md border-b border-white/5 h-16' : 'h-20'} flex items-center`}>
        <div className="max-w-7xl mx-auto px-6 w-full flex justify-between items-center text-white">
          ...
          <div className="hidden md:flex gap-8 items-center text-[11px] font-black uppercase tracking-widest text-gray-400">
            {['services', 'fleet', 'about', 'contact'].map(s => <button key={s} onClick={() => scrollToSection(s)} className="hover:text-white transition-colors">{t.nav[s as keyof typeof t.nav]}</button>)}
            ...
            <button onClick={() => scrollToSection('contact-section-container')} className="bg-[#0052ff] text-white px-5 py-2 rounded font-black hover:bg-[#003dd6] transition-all">{t.nav.getQuote}</button>
          </div>
      ...
      {isMenuOpen && (
          <div className="fixed inset-0 z-[100] bg-[#050a14] flex flex-col p-8 animate-fadeInUp">
            ...
            {['services','fleet','about','contact'].map(s => <button key={s} onClick={() => scrollToSection(s)} className="text-4xl font-black uppercase mb-8 text-left text-white">{t.nav[s as keyof typeof t.nav]}</button>)}
          </div>
      )}
      ...
      {/* FLEET */}
      <section id="fleet" className={`py-24 px-4 border-t ${borderColor} bg-[#050a14]`}>
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            <div className={`p-10 rounded-3xl border border-white/5 ${bgCard} text-left`}>
              ...
              <div className="space-y-4 mb-8">
                {[[t.fleet.length, '13.6 m'], [t.fleet.height, '2.7 m'], [t.fleet.capacity, '33 Euro Pallets'], [t.fleet.idealFor, t.fleet.generalCargo]].map(([l, v], i) => (
                  <div key={i} className="flex justify-between border-b border-white/5 pb-2 text-[10px] font-black uppercase tracking-widest text-gray-500"><span>{l}</span><span className="text-white font-black">{v}</span></div>
                ))}
              </div>
            ...
            <div className={`p-10 rounded-3xl border border-[#0052ff]/30 ${bgCard} text-left`}>
              ...
              <div className="space-y-4 mb-8">
                {[[t.fleet.internalHeight, '3.0 m'], [t.fleet.volume, '105 m³'], [t.fleet.specialization, t.fleet.lightweightCargo], [t.fleet.advantage, '+14% Capacity']].map(([l, v], i) => (
                  <div key={i} className="flex justify-between border-b border-white/5 pb-2 text-[10px] font-black uppercase tracking-widest text-gray-500"><span>{l}</span><span className="text-white font-black">{v}</span></div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-10">
            {[{icon:Shield,t:t.fleet.cmrInsured},{icon:Clock,t:t.fleet.realTime},{icon:Truck,t:t.fleet.modernFleet},{icon:Check,t:t.fleet.euro6}].map((item, i) => (
              <div key={i} className={`p-5 border border-white/5 ${bgCard} rounded-2xl`}><item.icon className="text-[#0052ff] mb-2 mx-auto" size={24}/><p className="font-black text-[10px] uppercase text-white">{item.t}</p></div>
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="services" className={`py-24 px-4 border-t ${borderColor} text-center bg-[#0a1628]`}>
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl font-black mb-16 uppercase text-white">{t.services.title}</h2>
            ...
            {t.services.items.map((s:any, i:number) => {
              ...
              return (
                <div key={i} className="p-8 rounded-3xl border border-white/5 bg-[#050a14] hover:border-[#0052ff] transition-all group">
                  ...
                  <p className="text-gray-600 text-[9px] mt-2 font-bold uppercase tracking-widest">{s.subtitle}</p>
                </div>
              )
            })}
          </div>
      ...
      {/* ABOUT (Полный текст + 8 карточек) */}
      <section id="about" className={`py-24 px-4 border-t ${borderColor} text-center bg-[#050a14]`}>
        <div className="max-w-5xl mx-auto">
          <h2 className={`text-4xl md:text-5xl font-black mb-10 uppercase`}>{t.about.title}</h2>
          <div className="text-gray-400 text-lg leading-relaxed mb-12 space-y-6">
             <p>{lang === 'en' ? 'TERSIS provides reliable, cost-effective transportation solutions across Europe and worldwide. We specialize in asset-based logistics, operating a modern fleet of 27 vehicles.' : 'TERSIS teikia patikimus transporto sprendimus visoje Europoje ir pasaulyje. Mes specializuojamės nuosavo transporto logistikoje, valdydami 27 modernių automobilių parką.'}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             {[ 
               { icon: Truck, l: 'Own Fleet' }, { icon: Globe, l: 'Routes' }, { icon: Clock, l: 'Reliability' }, { icon: Shield, l: 'Safety' },
               { icon: FileText, l: 'Pricing' }, { icon: Handshake, l: 'Partners' }, { icon: Users, l: 'Expert Team' }, { icon: Check, l: '100% Insured' }
             ].map((item, i) => (
               <div key={i} className={`p-4 ${bgCard} rounded-xl border border-white/5 hover:border-[#0052ff]/30 transition-colors`}>
                 <item.icon className="text-[#0052ff] mb-2 mx-auto" size={24} />
                 <p className="font-black text-[10px] uppercase text-white tracking-tighter">{item.l}</p>
               </div>
             ))}
          </div>
        </div>
      ...
      <section id="contact-section-container" className={`py-32 px-4 bg-[#050a14]`} onFocus={() => { isFocusing.current = true }} onBlur={() => { isFocusing.current = false }}>
      ...
      <footer className={`${bg} border-t ${borderColor} py-20 px-6`}>
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-16 text-left">
          <div className="space-y-6">
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({top:0, behavior:'smooth'})}><img src="https://tersis.lt/logo.png" alt="Logo" className="h-8" /><span className="text-xl font-bold uppercase text-white tracking-tighter">TERSIS</span></div>
