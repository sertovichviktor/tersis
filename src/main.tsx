...
// Проверь этот путь! Если App.tsx в папке src, а i18n.ts в src/lib, то './lib/i18n'
...
export default function TersisApp() {
  ...
  // Логика из рабочего кода (без лагов)
  ...
  // Если переводы не подгрузились, показываем заглушку, чтобы не упал React
  ...
  return (
    <div className={`min-h-screen ${bg} relative overflow-hidden transition-colors duration-300`}>
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? `${navBg} backdrop-blur-md border-b ${borderColor} shadow-sm` : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-18 py-4">
            <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="flex items-center group focus:outline-none">
              <div className="relative h-10 w-10"><img src="/logo.png" alt="TERSIS" className="h-full w-full object-contain transition-transform duration-300 group-hover:scale-110" /></div>
        ...
        <div className="max-w-7xl mx-auto w-full relative z-20">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="animate-fadeInUp text-left">
              ...
              <p className="text-lg text-gray-200 mb-10 leading-relaxed max-w-lg font-medium">{lang === 'en' ? 'We operate a fleet of 27+ modern Euro 6 vehicles, specializing in high-capacity MEGA trailers (105 m³) and standard transport worldwide.' : 'Valdome 27+ modernių Euro 6 transporto priemonių parką, specializuojamės MEGA puspriekabėmis.'}</p>
              <div className="flex flex-wrap gap-4">
                <button onClick={() => scrollToSection('contact')} className="bg-[#0052ff] text-white px-8 py-4 rounded-md text-base font-bold hover:bg-[#003dd6] transition flex items-center gap-2 uppercase tracking-wide shadow-lg">{t.hero.getQuote} <ArrowRight className="h-4 w-4" /></button>
      ...
      {/* 4. FLEET SECTION (Все таблицы из 10 билда) */}
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            ...
            <div className={`border ${borderAccent} p-6 md:p-8 ${bgCard} rounded-xl flex flex-col`}>
              <div className="flex items-center gap-4 mb-8">
                ...
                <div><h3 className={`text-xl font-black ${textPrimary} uppercase`}>{t.fleet.standardClass}</h3><p className="text-[#0052ff] font-bold text-sm">92 m³ {t.fleet.capacity}</p></div>
              </div>
              <div className="space-y-4 mb-8 flex-grow">
                {[[t.fleet.length, '13.6 m'], [t.fleet.height, '2.7 m'], [t.fleet.capacity, `33 ${t.fleet.euroPallets}`]].map(([l, v], i) => (
                  <div key={i} className={`flex justify-between border-b ${borderColor} pb-3`}><span className={`${textMuted} text-xs font-bold uppercase tracking-widest`}>{l}</span><span className={`${textPrimary} font-black uppercase`}>{v}</span></div>
                ))}
              </div>
            ...
            <div className={`border ${borderAccent} p-6 md:p-8 ${bgCard} rounded-xl flex flex-col`}>
              <div className="flex items-center gap-4 mb-8">
                ...
                <div><h3 className={`text-xl font-black ${textPrimary} uppercase`}>{t.fleet.megaAdvantage}</h3><p className="text-[#0052ff] font-bold text-sm">105 m³ {t.fleet.capacity}</p></div>
              </div>
              <div className="space-y-4 mb-8 flex-grow">
                {[[t.fleet.internalHeight, '3.0 m'], [t.fleet.volume, '105 m³'], [t.fleet.advantage, '+14% ' + t.fleet.capacity]].map(([l, v], i) => (
                  <div key={i} className={`flex justify-between border-b ${borderColor} pb-3`}><span className={`${textMuted} text-xs font-bold uppercase tracking-widest`}>{l}</span><span className={`${textPrimary} font-black uppercase`}>{v}</span></div>
                ))}
              </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[ 
              ...
              { icon: Users, t: 'Team', s: 'Professional' }, { icon: Check, t: 'CMR', s: '100% Insured' }
            ].map((item, idx) => (
              <div key={idx} className={`border ${borderAccent} p-6 ${bgCard} rounded-xl hover:border-[#0052ff]/50 transition-all group text-left`}>
                ...
                <p className={`text-base ${textMuted} uppercase text-xs font-bold`}>{item.s}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 6. COVERAGE MAP (Анимированные кометы) */}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative h-[400px] md:h-[650px] rounded-[40px] overflow-hidden border border-[#0052ff]/30 shadow-2xl bg-black">
            ...
            <svg className="absolute inset-0 w-full h-full z-10" viewBox="0 0 1000 600">
               ...
               <path d="M 150,230 Q 300,100 485,205" stroke="url(#comet-grad)" strokeWidth="2" fill="none" strokeDasharray="1, 50"><animate attributeName="stroke-dashoffset" from="300" to="0" dur="3s" repeatCount="indefinite" /></path>
               <path d="M 850,380 Q 650,250 485,205" stroke="url(#comet-grad)" strokeWidth="2" fill="none" strokeDasharray="1, 50"><animate attributeName="stroke-dashoffset" from="300" to="0" dur="2.5s" repeatCount="indefinite" /></path>
            </svg>
            <div className="absolute bottom-6 left-8 bg-black/40 backdrop-blur-md px-4 py-2 rounded-lg border border-white/10 hidden md:block">
              <p className="text-[10px] font-black text-[#0052ff] uppercase mb-1">Hub Status</p>
      ...
      {/* 7. CONTACT SECTION (Рабочая логика) */}
      ...
      <footer className={`${bg} border-t ${borderColor} py-16 px-4`}>
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-12 text-sm uppercase font-bold">
          <div><div className="flex items-center mb-6"><img src="/logo.png" className="h-10 w-10 mr-3" /><span className={`text-xl font-black ${textPrimary}`}>TERSIS</span></div><p className={textSecondary}>Taikos pr. 141-305, Kaunas, LT-51132</p></div>
          <div><h5 className={`font-black ${textPrimary} mb-6 tracking-widest`}>{t.footer.servicesTitle}</h5><div className={`space-y-2 ${textSecondary}`}>{t.services.items.slice(0, 4).map((svc: any, i: number) => <p key={i}>{svc.title}</p>)}</div></div>
          <div className="text-right"><p className="text-[#0052ff] font-black text-2xl mb-2">LIC-009666-EBKR</p><p className="text-green-400 font-bold tracking-widest"><Check className="h-4 w-4 inline mr-2" /> 100% CMR Insured</p></div>
        </div>
      </footer>
    </div>
  )
