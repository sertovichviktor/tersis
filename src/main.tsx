    ...
    <div className={`min-h-screen ${bg} relative overflow-hidden transition-colors duration-300`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-18 py-4">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <button onClick={() => scrollToSection('contact')} className="bg-[#0052ff] text-white px-5 py-2 hover:bg-[#003dd6] transition font-bold text-sm uppercase tracking-wide rounded-md">
                {t.nav.getQuote}
              </button>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="max-w-4xl mx-auto mb-16 space-y-6 text-center">
            <p className={`${textSecondary} text-lg leading-relaxed`}>
              {lang === 'en' ? 'TERSIS provides reliable, cost-effective transportation solutions across Europe and worldwide. We specialize in asset-based logistics, operating a modern fleet of 27 vehicles to ensure direct control and maximum efficiency.' : 'TERSIS teikia patikimus ir ekonomiškus transporto sprendimus Europoje bei visame pasaulyje.'}
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[ { icon: Truck, title: 'Own Fleet', sub: 'Full Control' }, { icon: Globe, title: 'Optimized Routes', sub: 'Efficient Schemes' }, { icon: Clock, title: 'High Reliability', sub: 'Strict Punctuality' }, { icon: Shield, title: 'Cargo Safety', sub: 'Guaranteed Protection' }, { icon: FileText, title: 'Transparent Pricing', sub: 'No Hidden Fees' }, { icon: Handshake, title: 'Sea & Air Partners', sub: 'Global Reach' }, { icon: Users, title: 'Professional Team', sub: '15+ Years Experience' }, { icon: Check, title: '100% CMR Insured', sub: 'Risk-Free Shipping' } ].map((item, idx) => (
      ...
      <section id="coverage" className={`py-24 px-4 sm:px-6 lg:px-8 border-t ${borderColor}`}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 uppercase">
            ...
            <p className="text-lg md:text-xl text-[#0052ff] font-black tracking-[0.3em]">Europe • Baltics • Global</p>
          </div>
          <div className="relative h-[400px] md:h-[650px] rounded-[30px] md:rounded-[40px] overflow-hidden border border-[#0052ff]/30 shadow-2xl bg-black">
            <img src="/map-hub.jpg.png" alt="Tersis Map" className="absolute inset-0 w-full h-full object-cover opacity-90" />
            <svg className="absolute inset-0 w-full h-full z-10 pointer-events-none" viewBox="0 0 1000 600">
              ...
              <g fill="none" strokeWidth="2">
                {["M 150,230 Q 300,100 485,205", "M 220,480 Q 350,300 485,205", "M 850,380 Q 650,250 485,205", "M 820,130 Q 650,80 485,205"].map((d, i) => (
                  <path key={i} d={d} stroke="url(#comet-grad)" strokeDasharray="1, 50">
                    <animate attributeName="stroke-dashoffset" from="300" to="0" dur={`${2.8 + i * 0.2}s`} repeatCount="indefinite" />
                  </path>
                ))}
              </g>
            </svg>
          ...
          <div className={`${bgCard} border ${borderAccent} rounded-2xl p-8 md:p-10 shadow-2xl`}>
            <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-5">
              {['from', 'to', 'cargoType', 'weight', 'volume', 'name'].map((key) => (
                <div key={key}>
                  <label htmlFor={key} className={`block text-xs font-bold ${textSecondary} mb-2 uppercase tracking-widest`}>{(t.contact as any)[key]}</label>
                  <input id={key} name={key} type="text" required autoComplete="off" className={`w-full px-4 py-3 ${inputBg} border ${borderAccent} ${textPrimary} focus:border-[#0052ff] outline-none transition rounded-lg text-sm font-bold`} placeholder={(t.contact.placeholders as any)[key]} />
                </div>
              ))}
      ...
      <footer className={`${bg} border-t ${borderColor} py-16 px-4 sm:px-6 lg:px-8`}>
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-12">
          <div>
            <div className="flex items-center mb-6"><div className="h-10 w-10"><img src="/logo.png" alt="TERSIS" className="h-full w-full object-contain" /></div><h4 className={`text-xl font-black ${textPrimary} ml-3`}>TERSIS</h4></div>
            <div className={`space-y-4 ${textSecondary} text-sm`}>
              <div className="flex items-start gap-3"><MapPin className="h-5 w-5 text-[#0052ff] flex-shrink-0" /><p>Taikos pr. 141-305, Kaunas, LT-51132, Lithuania</p></div>
              <div className="flex items-start gap-3"><Phone className="h-5 w-5 text-[#0052ff] flex-shrink-0" /><a href="tel:+37065955956">+370 65 955 956</a></div>
              <div className="flex items-start gap-3"><Mail className="h-5 w-5 text-[#0052ff] flex-shrink-0" /><a href="mailto:info@tersis.lt">info@tersis.lt</a></div>
            </div>
          </div>
          <div>
            ...
            <ul className={`space-y-2 ${textSecondary} text-sm`}>
              {t.services.items.slice(0, 5).map((svc, i) => ( <li key={i}><button onClick={() => scrollToSection('services')} className="hover:text-[#0052ff] transition">{svc.title}</button></li> ))}
            </ul>
          </div>
          <div>
