...
// --- ПОЛНЫЕ ПЕРЕВОДЫ (ВШИТЫ ДЛЯ ГАРАНТИИ) ---
const t: any = {
  en: {
...
// --- ИЗОЛЯЦИЯ ТЯЖЕЛЫХ БЛОКОВ (Чтобы не вешало сайт при клике) ---
const HeroSection = memo(({ lang, scrollTo }: any) => (
  <section className="relative h-screen flex items-center px-6 overflow-hidden bg-[#050a14]">
    ...
    <div className="max-w-7xl mx-auto w-full relative z-20 grid md:grid-cols-2 gap-12 items-center">
      <div className="animate-fadeInUp text-left">
        <div className="inline-block mb-6 px-4 py-2 bg-blue-600/10 border border-blue-600/20 rounded text-blue-500 text-[10px] font-bold tracking-widest uppercase">EST. 2011 • TRUSTED EXPERIENCE</div>
        <h1 className="text-5xl md:text-8xl font-black text-white leading-[0.85] uppercase mb-8 tracking-tighter">
          {t[lang].hero.title1}<br/>{t[lang].hero.title2}<br/><span className="text-blue-600">{t[lang].hero.title3}</span><br/>{t[lang].hero.title4}
        </h1>
        <p className="text-gray-300 text-lg mb-10 max-w-lg leading-relaxed">{t[lang].hero.desc}</p>
      ...
      <div className="hidden md:flex flex-col gap-6 items-end">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-2xl w-64 text-center">
          <FileText className="text-blue-600 mb-4 mx-auto" size={32}/><p className="text-xl font-black text-white uppercase">LIC-009666-EBKR</p>
        </div>
      </div>
    </div>
  </section>
...
const MapSection = memo(() => (
  <section id="coverage" className="py-32 px-8 bg-[#050a14] text-center border-t border-white/5">
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050a14] text-white font-sans antialiased overflow-x-hidden">
      ...
      <HeroSection lang={lang} t={t[lang]} scrollTo={scrollTo} />

      {/* FLEET */}
      <section id="fleet" className="py-32 px-8 bg-[#050a14]">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-5xl md:text-7xl font-black mb-4 uppercase tracking-tighter">{t[lang].fleet.title}</h2>
          <p className="text-gray-500 font-bold uppercase tracking-widest mb-20">{t[lang].fleet.subtitle}</p>
          <div className="grid md:grid-cols-2 gap-10 text-left">
            <div className="form-card p-10 rounded-[32px]">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h3 className="text-3xl font-black uppercase text-white">{t[lang].fleet.std}</h3>
              </div>
              ...
              <p className="text-gray-500 italic text-xs">{t[lang].fleet.standardFooter}</p>
            </div>
            <div className="form-card p-10 rounded-[32px] border-blue-600/20">
              <div className="flex items-center gap-4 mb-8">
                <div className="p-4 bg-blue-600/10 rounded-2xl border border-blue-600/20"><Maximize2 className="text-blue-600" size={32}/></div>
                <h3 className="text-3xl font-black uppercase text-blue-600">{t[lang].fleet.mega}</h3>
              </div>
              <div className="space-y-6 mb-10 text-[10px] font-black uppercase tracking-widest text-gray-500">
              ...
              <p className="text-gray-500 italic text-xs">{t[lang].fleet.megaFooter}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="services" className="py-32 px-8 bg-[#0a1628] border-y border-white/5 text-center">
        <h2 className="text-5xl font-black mb-20 uppercase tracking-tighter text-white">{t[lang].services.title}</h2>
      ...
      <section id="about" className="py-32 px-8 bg-[#050a14] text-center">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-5xl font-black mb-12 uppercase tracking-tighter">{t[lang].about.title}</h2>
          <div className="text-gray-400 text-lg leading-relaxed space-y-8 mb-20">
            ...
            <p className="text-white font-bold uppercase text-sm border border-white/10 inline-block px-4 py-2 rounded">License: LIC-009666-EBKR</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-left">
            {['OWN FLEET','CMR INSURED','EU NETWORK','24/7 SUPPORT','SECURE','EXPERT TEAM','FULL DOCS','TRUSTED'].map((l, i) => (
              <div key={i} className="p-8 bg-[#0a1628] rounded-[32px] border border-white/5 font-black text-[10px] uppercase text-white">{l}</div>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="form-card p-8 md:p-16 shadow-2xl">
            <form onSubmit={handleSubmit} className="space-y-8 text-left" autoComplete="off">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                <div className="space-y-2"><label className="text-[11px] font-black text-gray-500 uppercase tracking-widest">{t[lang].contact.from}</label><input name="from" required placeholder="Kaunas, Lithuania" className="w-full px-6 py-4 rounded-xl text-sm outline-none focus:border-blue-600 bg-[#0a1628]" /></div>
                <div className="space-y-2"><label className="text-[11px] font-black text-gray-500 uppercase tracking-widest">{t[lang].contact.to}</label><input name="to" required placeholder="Berlin, Germany" className="w-full px-6 py-4 rounded-xl text-sm outline-none focus:border-blue-600 bg-[#0a1628]" /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                <div className="space-y-2"><label className="text-[11px] font-black text-gray-500 uppercase tracking-widest">CARGO TYPE</label><input name="cargoType" placeholder="Electronics..." className="w-full px-6 py-4 rounded-xl text-sm outline-none focus:border-blue-600 bg-[#0a1628]" /></div>
                <div className="space-y-2"><label className="text-[11px] font-black text-gray-500 uppercase tracking-widest">WEIGHT (KG)</label><input name="weight" placeholder="5000" className="w-full px-6 py-4 rounded-xl text-sm outline-none focus:border-blue-600 bg-[#0a1628]" /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                <div className="space-y-2"><label className="text-[11px] font-black text-gray-500 uppercase tracking-widest">VOLUME (M³)</label><input name="volume" placeholder="10" className="w-full px-6 py-4 rounded-xl text-sm outline-none focus:border-blue-600 bg-[#0a1628]" /></div>
                <div className="space-y-2"><label className="text-[11px] font-black text-gray-500 uppercase tracking-widest">NAME</label><input name="name" required placeholder="John Doe" className="w-full px-6 py-4 rounded-xl text-sm outline-none focus:border-blue-600 bg-[#0a1628]" /></div>
              </div>
              <div className="text-white space-y-2"><label className="block text-[11px] font-black text-gray-500 uppercase tracking-widest">EMAIL ADDRESS</label><input name="email" type="email" required placeholder="john@company.com" className="w-full px-6 py-4 rounded-xl text-sm outline-none focus:border-blue-600 bg-[#0a1628]" /></div>
      ...
      <footer className="py-24 bg-[#050a14] border-t border-white/5 text-center px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-20 text-left">
          ...
          <div><h4 className="text-white font-black text-xs uppercase tracking-widest mb-6">Legal</h4><p className="text-gray-600 text-xs font-bold uppercase">License: LIC-009666-EBKR<br/>© 2026 TERSIS</p></div>
        </div>
      </footer>
    </div>
  )
